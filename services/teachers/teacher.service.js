...
const { teacherProfilePipeline, teacherAttendancePipeline } = require('../../helpers/commonAggregationPipeline.js')
...
module.exports = {
    requestProfileUpdate: async (teacherId, requestedFields) => {
        try {
            ...
            // ✅ Send email
            const emailSent = await sendEmail('teacher-profile-update-request', emailData);
    ...
    getProfile: async (teacherId) => {
        ...
        profile.profilePic = process.env.STATIC_URL + profile.profilePic
        profile.aadharFront = process.env.STATIC_URL + profile.aadharFront
    ...
    getAttendance: async (month, year) => {
        try {
            ...
            if (!mongoose.Types.ObjectId.isValid(teacherId)) return { success: false, message: 'TEACHER_ID_NOT_VALID' }
            if (!month || month < 1 || month > 12) return { success: false, message: 'MONTH_NOT_VALID' }
            if (!year || year < 1970 || year > 2100) return { success: false, message: 'YEAR_NOT_VALID' }

            const attendanceRecords = await TeacherAttendance.aggregate(teacherAttendancePipeline(teacherId, month, year));
            if (!attendanceRecords || attendanceRecords.length === 0) {
                return { success: true, message: 'NO_ATTENDANCE_RECORDS', attendance: [] };
            }

            return { success: true, message: 'FETCHING_ATTENDANCE_SUCCESSFULLY', attendance: attendanceRecords };
        } catch (err) {
